...
class GNN_LRP(WalkBase):
    ...
    def extract_step(self, x: Tensor, edge_index: Tensor, detach: bool = True, split_fc: bool = False):
        ...
        # walk_steps = []
        # current_input_for_walk_step = None
        # current_output_for_walk_step = None
        # in_message_passing_sequence = False

        # for layer_tuple in layer_extractor[3:]:
        #     module_instance = layer_tuple[0]
        #     module_output = layer_tuple[2]

        #     if isinstance(module_instance, GNNPool):
        #         break

        #     if isinstance(module_instance, MessagePassing):
        #         current_input_for_walk_step = module_input
        #         current_output_for_walk_step = module_output
            
        #     elif in_message_passing_sequence:
        #         current_modules_for_walk_step.append(module_instance)
        #         current_output_for_walk_step = module_output
            
        #     # else:

        #     if in_message_passing_sequence and isinstance(module_instance, nn.Dropout):
        #             step = {
        #                 'input': current_input_for_walk_step,
        #                 'output': current_output_for_walk_step
        #             }
        #             walk_steps.append(step)
                
        #         current_modules_for_walk_step = []
        #         in_message_passing_sequence = False
